...
def averageChange7Day(df):
    currentDate = df['Date'].head(1).tolist()
    # print(f'head: {df["Date"].head(1).tolist()}')
    ...
    # print(df.loc[index, 'Date'] - timedelta(days=7))
    workingDate = df.loc[index, 'Date'].date()
    totalRows = len(df.index)
    # print(totalRows)
    past7avg = 0
    while index < (totalRows-1) and workingDate >= past7:
        past7avg += df.loc[index, 'Weight(lb)']
        # print(workingDate)
        index += 1
        # print(f'totalRows: {totalRows}')
        # print(f'index: {index}')
        # print(f'index < totalRows: {index < totalRows}')
        # print(df.loc[index])
        # print(df.loc[index, 'Date'])
        # print(type(df.loc[index, 'Date']))
        workingDate = df.loc[index, 'Date'].to_pydatetime()
        workingDate = workingDate.date()
        # print(workingDate)
        # print(type(workingDate))
        # workingDate = df.loc[index, 'Date'].date()
    if index != 0:
    ...
    count = 0
    past14avg = 0
    while index < (totalRows-1) and workingDate >= past14:
        ...
        workingDate = df.loc[index, 'Date'].to_pydatetime()
        workingDate = workingDate.date()
    print(f'index: {index} index14Start: {index14Start}')
    if index != 0 and (index - index14Start != 0):
        past14avg = past14avg / (index - index14Start)
    print(f'past7avg: {past7avg} past14avg: {past14avg}')
...
def graphs(df):
    ...
    df_display_body_fat['Body Fat(%)'] = df_display_body_fat['Body Fat(%)'].astype(float)
    df_display_body_fat = df_display_body_fat.groupby('just_date').mean().reset_index()
    alt_graph_body_fat = alt.Chart(df_display_body_fat).mark_area(opacity=1).encode(x='just_date', y=alt.Y('Body Fat(%)', scale=alt.Scale(
        domain=(9, 12))), tooltip=['Body Fat(%)', 'just_date'])
    st.altair_chart(alt_graph_body_fat, use_container_width=True)
    ...
    #
    # avgLB.drop_duplicates(subset="just_date", keep='first', inplace=True)
    # st.dataframe(avgLB)
    # st.area_chart(avgLB.rename(columns={'just_date': 'index'}).set_index('index'))
...
def jeffSynced():
    ...
    # df['Date'] = pd.to_pydatetime(df['dateTime'])
    df = pd.merge(df, df_body_fat, how='outer')
    df.rename(columns={'value': 'Body Fat(%)'}, inplace=True)
